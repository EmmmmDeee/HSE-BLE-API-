...
| history-persistence | A per-device timeline (first/last seen, recurrence) persisted across sessions | BlueHydra / WiGLE | — | — | ABSENT | Not implemented; identity correlation is the prerequisite and now exists |
| identity-correlation | Correlate one physical device across BLE address rotation with explicit uncertainty | BlueHydra | bleradar_core::identity | NativeRadar.deviceGroupKey | PARTIAL | No cross-session history yet; U/L-bit address-type approximation; app does not yet visually merge rows |
...
Totals: 0 SUPERIOR, 2 PARITY, 5 PARTIAL, 2 ABSENT, 1 BLOCKED_BY_PLATFORM, 0 UNVERIFIED.
