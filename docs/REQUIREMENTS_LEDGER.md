...
| REQ-CORE-010 | Spatial estimate requires ≥2 positioned observations and averages position on the sphere (3-D unit-vector centroid weighted by accuracy, relative signal strength, and an exponential recency decay so recent fixes dominate a long-lived track), not linearly in degrees, so antimeridian-straddling observations do not produce a centroid on the wrong side of the planet; the reported uncertainty radius is the `max` (not the mean) of every contributing fix's own haversine-distance-to-centroid-plus-GPS-accuracy, so it is guaranteed to contain every fix's error disk, not just the typical one | `DeviceTrack` → `Option<SpatialEstimate>` (confidence + uncertainty) | Pure read; `None` below the minimum sample count | `tracking.rs::DeviceTrack::spatial_estimate`, `observation_weight`, `confidence_from_accuracy` | `core.rs::spatial_estimate_requires_multiple_positioned_observations`, `spatial_estimate_handles_antimeridian_straddling`, `spatial_estimate_prefers_recent_position_fixes`, `spatial_estimate_uncertainty_contains_every_fix_error_radius`, `map_point_confidence_preserves_accuracy_tiers`; `properties.rs::prop_track_push_and_spatial_estimate_stay_valid` (forces antimeridian straddling periodically) | VERIFIED |
...
| REQ-CORE-013 | Tracking exposes both legacy RSSI proximity and calibrated-distance proximity without conflating their input semantics | tracking inputs → `TrackingSnapshot::{proximity, distance_proximity}` | RSSI classification remains backward-compatible; unrepresentable distance yields `None`, and JNI/Android maps it conservatively to `Far` | `tracking.rs::tracking_snapshot`; `bleradar-jni::tracking_distance_proximity_ordinal`; `NativeRadar.trackingDistanceProximity` | `core.rs::tracking_snapshot_keeps_distance_proximity_distinct_from_rssi_proximity`; `jni_bridge.rs::distance_proximity_ordinal_uses_calibrated_boundaries`, `tracking_snapshot_uses_invalid_sentinels`; live JNI/Android proof | VERIFIED |
| REQ-CORE-014 | Calibrated distance estimates prefer a device's own advertised/calibrated TX power over the generic per-profile reference power when present and plausible, and ignore it (falling back to the profile constant) otherwise, including Android's `ScanResult.TX_POWER_NOT_PRESENT` sentinel | `(tx_power_dbm: Option<f64>, profile_rssi_at_1m_dbm)` → `f64`; threaded through `TrackingSnapshotInput::tx_power_dbm` | Pure; absent/non-finite/out-of-plausible-range values fall back to the profile constant without corrupting the estimate | `signal.rs::effective_rssi_at_1m_dbm`; `tracking.rs::tracking_snapshot`; `bleradar-jni::TrackingSnapshotJniInput`/`tracking_distance_m_or_nan`; `NativeRadar.tracking*(..., txPowerDbm)`; `BleScanEngine.readTxPowerDbm` | `core.rs::effective_rssi_at_1m_dbm_prefers_plausible_tx_power`, `tracking_snapshot_prefers_plausible_device_tx_power_over_profile_calibration`, `tracking_snapshot_ignores_implausible_or_absent_device_tx_power`; `jni_bridge.rs::tracking_distance_prefers_plausible_device_tx_power_over_profile_calibration`; live JNI/Android proof (ABI 7) | VERIFIED |

**Runtime verification evidence:** `cargo test -p bleradar-core --locked --test core --test properties` and `cargo test -p bleradar-jni --locked --test jni_bridge` (2026-09-09, this session). Doctests and workspace integration are covered by `cargo xtask gates`. REQ-CORE-014 added 2026-09-09 (later same day): `cargo test -p bleradar-core --test core` → 37 passed (up from 34), `cargo test -p bleradar-jni --test jni_bridge` → 15 passed (up from 14); `cargo xtask gates` (fmt/clippy/build/test/doc/parity/dependency-policy/oracle-integrity/audit/deny) all green; `cargo xtask verify-jni-live` (failure-path `UnsatisfiedLinkError` + success-path `abi=7`, including a live assertion that a plausible device TX power actually overrides the profile-only distance estimate and that the `TX_POWER_NOT_PRESENT` sentinel is ignored); `cargo xtask verify-android-live` (cross-compiled/signed/aligned APK, `libbleradar_jni.so` verified present). `bleradar-core` bumped 0.5.0→0.6.0 (new public `TrackingSnapshotInput::tx_power_dbm` field is an additive breaking change for struct-literal construction); JNI ABI bumped 6→7. REQ-CORE-010 reworded 2026-09-09 (still later same day, decision #53): reconciling a Copilot PR review finding, no code changed — `spatial_estimate_prefers_recent_position_fixes`, `spatial_estimate_uncertainty_contains_every_fix_error_radius`, and `map_point_confidence_preserves_accuracy_tiers` already existed in `core.rs` (added alongside REQ-CORE-013 in the same PR) but were never cited here.
...
**Runtime verification evidence:** `cargo test -p bleradar-core --locked --test evidence` → 11 passed, 0 failed (2026-09-02). REQ-EVID-009/010 reclassified `VERIFIED` in this session's ledger reconciliation: the closing tests already existed in `evidence.rs` (11 tests, up from the 7 this ledger had recorded) and were simply never cross-checked against these two rows. Random-operation campaign added 2026-09-10 (decision #62): `tests/evidence_campaign.rs` drives random sequences of every insert/update over a colliding id space and checks no-panic, no mutation on rejection, `validate()` consistency (tolerating only the documented claim-before-evidence incompleteness, cross-checked with `trace_claim`), and `observations_by_source` recounts; 5,000,000 operations in the scratch run and a deterministic 25,000-operation sample per `cargo test`, zero violations.
...
**Runtime verification evidence:** `cargo test -p bleradar-core --locked --test osint` → 13 passed, 0 failed (2026-09-02). REQ-OSINT-001/009/010 reclassified `VERIFIED` in this session's ledger reconciliation: the closing tests already existed in `osint.rs` (13 tests, up from the 10 this ledger had recorded) and were simply never cross-checked against these rows. Random-operation campaign added 2026-09-10 (decision #63): `tests/osint_campaign.rs` checks limits, single execution per pivot, refusal-without-mutation, fresh `Proposed` generated pivots with parents, persisted findings/actions, a unique `Proposed`-only ranking consistent with `next_pivot`/`has_ready_pivot`, family-statistics totals, and evidence-store validity after every operation; 3,000,000 operations in the scratch run and a deterministic 6,000-operation sample per `cargo test`, zero violations.
...
**Runtime verification evidence:** `cargo test -p bleradar-core --locked --test website` → 13 passed, 0 failed (2026-09-04). REQ-WEB-004 was fully closed in an earlier session; REQ-WEB-002 was fully verified in this session: `website.rs` test count grew from 9 to 13, adding direct leading-outcome tests for `CommonTemplate`, `ContentReuse`, `DevelopmentRelationship`, and `Coincidence` (`common_template_leads_for_html_structure_matches`, `content_reuse_leads_for_distinctive_phrase_matches`, `development_relationship_leads_for_script_reference_matches`, and `coincidence_leads_when_multiple_disjoint_named_explanations_compete_without_corroboration`).

---

## REQ-PIPE — Investigation pipeline (control-loop graph + stop criterion)

`crates/bleradar-core/src/pipeline.rs`, `crates/bleradar-core/tests/pipeline.rs`

| ID | Requirement | Inputs → Outputs | Side effects / Failure behavior | Location | Tests | Status |
|---|---|---|---|---|---|---|
| REQ-PIPE-001 | All sixteen directive stages are named in order with a total, cyclic successor function and a `Display` matching the exact directive wording | `PipelineStage` variant → ordinal / next / rendered text | Pure | `pipeline.rs::PipelineStage` | `pipeline_stage_ordinal_matches_all_array_position`, `pipeline_stage_next_is_linear_through_the_whole_directive`, `pipeline_stage_next_cycles_back_from_stop_to_generate_pivots`, `pipeline_stage_display_matches_the_directive_wording` | VERIFIED |
| REQ-PIPE-002 | Graph node/edge construction rejects empty ids/labels and edges referencing unknown nodes | node id / edge endpoints → `TemporalGeoGraph` or `PipelineError` | No partial mutation on rejection | `pipeline.rs::TemporalGeoGraph::add_node/add_edge` | `add_node_rejects_empty_id`, `add_edge_rejects_unknown_endpoints`, `add_edge_rejects_empty_label` | VERIFIED |
| REQ-PIPE-003 | Re-adding a known node id upserts rather than duplicates, preserving its position when none is supplied and replacing it when one is | node id (+ optional position) → unchanged node count | Idempotent upsert | `pipeline.rs::TemporalGeoGraph::add_node` | `add_node_upsert_preserves_position_when_re_added_without_one`, `add_node_upsert_updates_position_when_re_added_with_a_new_one` | VERIFIED |
| REQ-PIPE-004 | Connected components ("clusters") are reported deterministically, including isolated singleton nodes | graph → sorted `Vec<Vec<NodeId>>` | Pure | `pipeline.rs::TemporalGeoGraph::clusters` | `clusters_groups_disjoint_components_and_isolates_singletons` | VERIFIED |
| REQ-PIPE-005 | Bridge edges (cut-edges whose removal disconnects the graph) are found correctly, including on multigraphs with parallel edges between the same two nodes | graph → `Vec<(NodeId, NodeId)>` | Pure; iterative (non-recursive) Tarjan bridge search | `pipeline.rs::TemporalGeoGraph::bridges` | `bridges_reports_every_edge_of_a_simple_path`, `bridges_reports_none_for_a_triangle_cycle`, `bridges_finds_only_the_single_edge_joining_two_triangles`, `bridges_ignores_parallel_edges_between_the_same_two_nodes`. Both the two-triangles and parallel-edges cases were confirmed discriminating by deliberately reintroducing the classic parent-node-instead-of-parent-edge multigraph bug and observing the test fail before restoring the fix (2026-09-06 session). | VERIFIED |
| REQ-PIPE-006 | A cluster's temporal span is the min/max `observed_at` over edges with both endpoints in that cluster, or absent when none qualify | cluster member ids → `Option<TemporalSpan>` | Pure integer min/max, no float subtlety | `pipeline.rs::TemporalGeoGraph::cluster_temporal_span` | `cluster_temporal_span_covers_only_edges_within_the_cluster`, `cluster_temporal_span_is_none_for_a_cluster_with_no_qualifying_edge` | VERIFIED |
| REQ-PIPE-007 | The loop stops only once every round in a full sliding window yielded at or below the minimum gain threshold; a single high-gain round anywhere in the window keeps it going, and stops it again once that round slides out | round gain samples → `should_stop() : bool` | Rejects `window == 0` construction | `pipeline.rs::DiminishingGainStopCriterion` | `stop_criterion_rejects_zero_window`, `stop_criterion_does_not_stop_before_the_window_fills`, `stop_criterion_stops_once_every_round_in_the_window_is_low_gain`, `stop_criterion_keeps_going_while_any_round_in_the_window_is_high_gain`, `stop_criterion_recovers_once_a_high_gain_round_slides_out_of_the_window`. The all-vs-any distinction was confirmed discriminating by deliberately swapping `all()` for `any()` and observing the test fail before restoring the fix (2026-09-06 session). | VERIFIED |
| REQ-PIPE-008 | The orchestrator only progresses stages linearly via `advance`, only evaluates the stop criterion via `complete_round` at the stop-check stage, and refuses all further progress once stopped | stage/round calls → next stage or `PipelineError` | `WrongStage`/`Stopped` on misuse; no state change on error | `pipeline.rs::InvestigationPipeline` | `investigation_pipeline_new_propagates_invalid_window`, `investigation_pipeline_starts_at_discover`, `investigation_pipeline_advance_reaches_the_stop_check_stage_in_order`, `investigation_pipeline_advance_at_the_stop_check_stage_is_rejected`, `investigation_pipeline_complete_round_elsewhere_is_rejected`, `investigation_pipeline_complete_round_loops_back_when_gain_is_still_high`, `investigation_pipeline_stops_after_enough_low_gain_rounds_and_then_rejects_further_calls` | VERIFIED |

**Runtime verification evidence:** `cargo test -p bleradar-core --locked --test pipeline` → 26 passed, 0 failed; `cargo test -p bleradar-core --locked` (in-module `pipeline::tests`) → 4 passed, 0 failed (2026-09-06). `cargo test --workspace --locked` shows no regressions elsewhere. `cargo clippy --workspace --all-targets --locked -- -D warnings` and `RUSTDOCFLAGS="-D warnings" cargo doc --workspace --no-deps --locked` both pass cleanly. Differential oracle added 2026-09-10 (decision #61): `clusters_and_bridges_match_a_brute_force_reference_on_random_multigraphs` compares `clusters()`/`bridges()` with a union-find reference (an edge is a bridge iff removing it increases the component count) on a deterministic 2,000-graph sample of random multigraphs with self-loops and parallel edges; the same oracle matched on 50,000 graphs in the scratch run. `cargo test -p bleradar-core --locked --test pipeline` → 27 passed.

---

## REQ-HSE — Imported entity model and coordinate parser (`entity.rs`, `coords.rs`, `tags.rs`)

`crates/bleradar-core/src/{entity,coords,tags}.rs`, `crates/bleradar-core/tests/entity.rs`, `crates/bleradar-core/tests/falsification_campaign.rs`; provenance `docs/HSE_IMPORT.md`. These rows were absent until 2026-09-10: the import (PR #12) carried its ported test suite but no requirement rows, which is itself the kind of silent gap this ledger exists to prevent.

| ID | Requirement | Inputs → Outputs | Side effects / Failure behavior | Location | Tests | Status |
|---|---|---|---|---|---|---|
| REQ-HSE-001 | `coords::parse` is total and panic-free for arbitrary input; every `Some` is finite and in range; Plus Code and Maidenhead digits are ASCII only, so no non-ASCII character can decode as a digit | any `&str` → `Option<ParsedCoordinate>` | `None` for anything unparsable, out of range, or forged; never panics | `coords.rs::parse`, `olc_index`, `field_index`, `subsquare_index` | `coordinate_parse_is_total_and_in_range`, `parse_plus_code_full_codes_only`, `parse_plus_code_rejects_non_ascii_homoglyph_digits`, `parse_maidenhead_locators`; campaign: 3 seeds × 5,000,000 iterations, zero violations | VERIFIED (2026-09-10; was BROKEN for forged Plus Codes — COR-020) |
| REQ-HSE-002 | `normalise(kind, value)` is a fixed point for every kind (`normalise(normalise(v)) == normalise(v)`), so one value can never fork onto two UIDs by being ingested twice | kind + raw value → canonical value | Pure | `entity.rs::normalise`, `normalise_url_query` | `normalise_is_idempotent_for_every_kind`, `normalise_is_idempotent_across_kinds`, `normalise_domain_is_a_fixed_point_when_whitespace_separates_www_labels`, `normalise_url_is_a_fixed_point_with_whitespace_before_a_trailing_slash`, `normalise_url_is_a_fixed_point_with_whitespace_inside_the_query`; campaign idempotence oracle over random and mutated inputs | VERIFIED (2026-09-10; was BROKEN for whitespace-corrupted Domain/Url values — COR-022/023) |
| REQ-HSE-003 | UIDs are deterministic: `uid_for(kind, raw) == derive_uid(kind, &normalise(kind, raw))`, SHA-256 over an unambiguous preimage | kind + value → hex UID | Pure | `entity.rs::derive_uid`, `uid_for`, `Sha256` | `uid_is_deterministic`, `uid_differs_across_kinds`, SHA-256 published vectors; campaign oracle checks the equality on every input | VERIFIED |
| REQ-HSE-004 | A `Coordinates` value is canonicalised to the 6-dp `"lat,lon"` form only when it passes the single `LatLon::new` validity gate (finite and in range), through either the bare-decimal fast path or `coords::parse`; anything else is left unchanged | raw string → canonical or unchanged string | Pure | `entity.rs::normalise` (`Coordinates` arm), `fmt_coord_6dp` | `normalise_coordinates_does_not_canonicalise_out_of_range_pairs`, `normalise_coordinates_rejects_non_finite`, `normalise_coordinates_same_point_same_uid_across_notations` | VERIFIED (2026-09-10; was BROKEN for out-of-range pairs — COR-021) |
| REQ-HSE-005 | `HseEntity` confidence, decay, `c_effective`, and merge stay within `[0, 1]` and finite for any timestamps (including `now < observed_at`) and any `f64` confidence input | entity + timestamps → scores | Pure; non-finite confidence sanitises to 0 | `entity.rs::HseEntity` | `c_eff_*` tests in `tests/entity.rs`; campaign oracle over random timestamps and `f64` bit patterns | VERIFIED |

**Runtime verification evidence:** `cargo test -p bleradar-core --locked --test entity` → 127 passed (2026-09-10). Falsification campaign (scratch harness identical in oracles to `tests/falsification_campaign.rs`): seeds 104372539623696, 987654321, 42 × 5,000,000 iterations each → 108,750,000 checks, zero violations after the COR-020..023 fixes; the same harness reported all four defects on the pre-fix code.
...
| REQ-XTASK-009 | `cargo xtask check-jni-contract [lib]` derives the expected JNI export set from `NativeRadar.java` (the single authority: `package`, `class`, every `static native` method, JNI name mangling) and requires the built library's `Java_*` exports to equal it exactly — no declared native without an export, no export without a declaration | Java source + ELF64 library → pass/fail naming every missing/orphaned symbol | Fails `gates`, `verify-jni-live`, and `verify-android-live`; rejects non-`static` natives and overloads explicitly; non-ELF host libraries fail with a message naming the limitation | `xtask/src/main.rs::JniExportContract`, `strip_java_comments_and_literals`, `java_package_and_class`, `java_static_native_method_names`, `jni_mangle`, `verify_jni_export_contract`, `cmd_check_jni_contract` | `strip_java_comments_and_literals_blanks_prose_but_keeps_code`, `java_package_and_class_read_the_declarations`, `java_static_native_method_names_are_sorted_and_multi_line_tolerant`, `java_static_native_method_names_rejects_non_static_and_overloaded_natives`, `jni_mangle_applies_the_specification_escapes`, `jni_export_contract_derives_prefix_and_exports_from_the_java_source`, `jni_export_contract_requires_an_exact_export_set`, `real_native_radar_facade_parses_into_a_non_empty_contract`; live: host library and the committed APK's `libbleradar_jni.so` both 21 ↔ 21; falsified by removing `#[unsafe(no_mangle)]` from one export, adding an orphan export, declaring an unexported Java native, and declaring a non-static native — each failed with the exact symbol/method named, then restored to exact match | VERIFIED (2026-09-10) |
| REQ-XTASK-010 | `cargo xtask verify-jni-live` resolves and invokes every native `NativeRadar.java` declares inside a real JVM (reflection over `Modifier.isNative`), and cross-checks that count against REQ-XTASK-009's parse | host library + façade → JVM run | An unexported native fails at link time inside the harness; a parser/reflection disagreement fails with both counts and the linked list | `xtask/src/main.rs::jni_smoke_java_source` (`verifyEveryDeclaredNativeLinks`) | `jni_smoke_java_source_embeds_expected_checks`; live `linked-natives=21` / `abi=7`; falsified by running the generated harness with the count patched to 20 (failed naming 21 linked natives) | VERIFIED (2026-09-10) |

**Runtime verification evidence:** `cargo test --manifest-path xtask/Cargo.toml --locked` → 47 passed, 0 failed (2026-09-02, this session; was 30 before this session's `repo_root_from` fix). `cargo xtask gates` → exit 0, all sub-gates green (2026-09-02, this session, after all fixes). 2026-09-10: `cargo test --manifest-path xtask/Cargo.toml --locked` → 75 passed (was 66); `cargo xtask gates` → exit 0 including the new `== jni export contract ==` gate; `cargo xtask verify-jni-live` → `linked-natives=21`, `abi=7`.
...
| REQ-PROC-006 | CI runs the live JVM → JNI → Rust proof (`cargo xtask verify-jni-live`) on every push/PR, on a pinned JDK, after `cargo xtask gates` | push/PR event → workflow run | Fails the check if the façade, the exports, or the JVM linkage drift | `.github/workflows/gates.yml` (`actions/setup-java@v4`, Temurin 21) | reproduced locally on OpenJDK 21.0.10 (`linked-natives=21`, `abi=7`); observed remotely on PR #19 head `7f28da0`, run 55 (https://github.com/EmmmmDeee/HSE-BLE-API-/actions/runs/34422048823): "Install pinned JDK" < 1 s, "Run every gate via the local gate runner" 18 s, "Live JVM → JNI → Rust proof against NativeRadar.java" 4 s, all `success` | VERIFIED (2026-09-10, remote run 55) |
| REQ-PROC-007 | The advisory/policy tools CI runs are pinned to exact versions and cached, so a gate result is reproducible run to run and does not depend on whatever `cargo install` resolves that day | workflow env `CARGO_AUDIT_VERSION`/`CARGO_DENY_VERSION` → cache key + install | Cache miss rebuilds exactly the pinned versions; a version bump changes the key | `.github/workflows/gates.yml` (`actions/cache@v4` on `~/.cargo/bin/cargo-{audit,deny}`) | the pinned versions (0.22.2 / 0.20.2) are the ones observed green locally (`cargo xtask gates`, 2026-09-10; a from-source build took 5m43s on this host, which is what the cache removes); remote run 55 (https://github.com/EmmmmDeee/HSE-BLE-API-/actions/runs/34422048823) restored the cache in 1 s (miss, first run with the new key), built the pinned tools in 4m39s, `cargo audit`/`cargo deny` passed inside `gates`, and the post step saved the cache in 2 s | VERIFIED (2026-09-10, remote runs 55 and 56: run 56 on head `f5620fd` (https://github.com/EmmmmDeee/HSE-BLE-API-/actions/runs/34422523485) restored the cache in 1 s (hit), skipped the build step, and completed the whole job in 35 s against run 55's 5m19s) |
...
| 17 | COR-020/021/022/023 (forged Plus Code digits; out-of-range coordinate fast path; Domain and Url canonical forms not fixed points) | `BROKEN` — found by the 2026-09-10 randomised falsification campaign (36.25 million checks on the unfixed crate); each regression test failed on the unfixed code | `VERIFIED` (REQ-HSE-001/002/004) | fixes in `coords.rs`/`entity.rs`, five regression tests, campaign rerun to zero violations across three seeds, `bleradar-core` 0.6.1, APK regenerated (decision #58) |
| 18 | Per-scan-result JNI cost had no measured baseline; the "one packed JNI call" candidate was argued, not measured | `IMPLEMENTED_UNVERIFIED` (no benchmark) | `VERIFIED` (measured) | `crates/bleradar-jni/examples/scan_result_cost.rs` + `benchmarks/README.md`: 129.2 ns per snapshot, 1,075.5 ns per eight-call scan result, candidate rejected on the numbers (decision #59) |
...
- **Reconstructed Android app**: REQ-ANDROID-002/003 are compile-verified and
  ...
  on a device or emulator (MIG-003). REQ-PROC-006/007 were closed on
  2026-09-10 by remote run 55 of the reworked `gates.yml`.
