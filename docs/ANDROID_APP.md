...
committed APK is the #97 build (SHA-256 `f049dce0…3aff`, 406,094 bytes; the
native library byte-identical to the #96 build; every entry stored at the
ZIP epoch, so a rebuild on one signing key is byte-identical, and
`verify-android-live` requires the committed entries to reproduce from the
sources — decision #97). The SDK set that build uses
