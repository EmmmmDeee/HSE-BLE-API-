...
| `ApiHttpServer.java` | Loopback-only (`127.0.0.1:8080`) HTTP/1.1 responder over `java.net.ServerSocket` (Android ships no `com.sun.net.httpserver`) serving the ranked snapshot as JSON (`/api/devices`, `/api/status`, `/api/updates`) and, at `/`, the web dashboard below, for a browser or Termux tooling on the same device; started and stopped with `RadarScanService`. It references nothing in `android.*` — collaborators arrive as `SnapshotSource`, `UpdateStatusSource`, `AssetSource` and two clocks, JSON comes from `Json`, logging from `java.util.logging` — so `cargo xtask verify-api-live` runs this exact class on a host JVM. |
| `SnapshotSource.java`, `UpdateStatusSource.java`, `AssetSource.java`, `Json.java` | The server's seams: the live scan state (`BleScanEngine` implements it), the update bookkeeping (`UpdateManager`), an asset opener (`getAssets()::open` on the device, a file on the host), and the minimal JSON writer that replaced `android.util.JsonWriter` (objects, arrays, names, the scalar values the API uses, RFC 8259 escaping, no whitespace). |
...
| The app's real `ApiHttpServer` runs on the host JVM with fixture sources and the host `bleradar-jni` library: ten real HTTP requests must be answered as documented (the dashboard bytes, the three JSON documents byte-identical to the browser fixtures, `404`/`405`/`400`, `no-store`, exact `Content-Length`), then headless Chromium must render the committed dashboard from that server | `cargo xtask verify-api-live` | a JDK (`javac`/`java`) and a Chromium/Chrome binary | yes (`gates` job, after `verify-jni-live`) |
...
committed APK is the #91 build (SHA-256 `3559634d…353e`, 406,094 bytes; the
