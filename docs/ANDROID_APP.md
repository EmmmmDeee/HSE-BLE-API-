...
| Source | Role |
|---|---|
| `NativeRadar.java` | The only JNI façade: `static native` declarations, ordinal/sentinel constants, `EXPECTED_ABI_VERSION`, and a never-throwing `ensureLoaded()`. It is the single authority for the export contract enforced by `cargo xtask check-jni-contract`. Since ABI 10 it also declares the four string-taking natives of the release-manifest / artifact surface, which the Rust side reads through the audited `crates/bleradar-jni/src/env.rs`; since ABI 11, `remoteManifestDisposition`, the Rust disposition of a remote-manifest fetch; since ABI 12, `deviceAddressTrackability`, the Rust classification of a device address as trackable hardware, a randomized (locally-administered, rotating) address, or unknown; since ABI 13, `advertisementCompanyId` and `advertisementBeacon`, summaries of the advertising payload (`ScanRecord.getBytes()` as hex) decoded by `bleradar_core::adv`; since ABI 14, `deviceGroupKey`, the rotating-address identity grouping key from `bleradar_core::group_key`; since ABI 15, `advertisementManufacturerName`, the Bluetooth SIG assignee name from `bleradar_core::adv::company_name`; since ABI 16, `advertisementServices`, the well-known service names from `bleradar_core::adv::service_uuid_name`; since ABI 17, `historyMerge` and `historyLookup`, the persistent cross-session device history from `bleradar_core::history`. |
...
| `DeviceHistory.java` | The persistent, cross-session device memory: loads `files/device_history.txt` (the app's private files directory — no permission, no configuration), batches sightings of public addresses (a new device within ~1 s, known ones every 10 s, since scan results arrive on the main thread), merges them through `NativeRadar.historyMerge` (Rust owns which keys are kept, what a visit is, the 2,048-device bound and damaged-file recovery), and writes the result atomically (temporary file + rename). `/api/devices` reports each remembered device's `first_seen_ms` and `visits`; it survives a process kill, a reboot and an upgrade (`verify-android-emulator` proves the kill). Uses `java.util.logging` so it also runs on the host JVM (`DeviceHistoryTest`). |
...
| `UpdateCheckService.java` | The automatic-update orchestration (`docs/AUTO_UPDATE.md`): throttle → the release manifest fetched from the repository's latest release on the service's worker thread (`ReleaseManifestSource`; the bundled manifest as the fallback, `NativeRadar.remoteManifestDisposition` deciding whether the fault deserves a retry) → `NativeRadar.updateDecision` → real network/battery/storage conditions → `NativeRadar.downloadReadiness` → `DownloadManager` → `NativeRadar.artifactVerifyFile` (the Rust `ArtifactVerifier`, streamed over the file) → system installer via the DownloadManager `content://` URI; retries paced by `NativeRadar.retryBackoffDelaySeconds`; always a `dataSync` foreground service, promoted first on every start because every start arrives through `startForegroundService()` (COR-030), and left through one `finish()` on every exit. |
| `ReleaseManifest.java` | A thin holder over the Rust core's validation: a text is accepted exactly when `bleradar_core::update::ReleaseManifest::parse` accepts it (`NativeRadar.releaseManifestCanonical`), every field is read back through `NativeRadar.releaseManifestField`, and `serialize()` is the canonical form Rust emitted. Java parses nothing. References nothing in `android.*` (logging through `java.util.logging`, which Android routes to logcat), so `cargo xtask verify-android-unit` runs `ReleaseManifestTest` against it on the host JVM with the real core. |
| `ReleaseManifestSource.java` | The platform half of learning about a release: GETs a manifest URL with timeouts and a 16 KiB cap (`Streams.readUpTo`), never throws, and reports the HTTP status, a `MANIFEST_FETCH_*` failure kind and a one-line detail; references nothing in `android.*`, so `cargo xtask verify-api-live` runs it on the host JVM against a scripted server. Every decision about the outcome is the Rust core's. |
...
- Evidence classification: the contract above is compile-verified (`javac`
  ...
  `kill -9` (the scan resumed 2.2 s later), the clean force-stop and — since
  decision #94 — a runtime permission revoked while scanning (the platform
  ...
  update check is observed fetching the repository's release manifest
  (since decision #96: `HTTP 404 -> using the bundled manifest; no retry`,
  the answer for a repository without releases), reaching its decision and
  finishing. Since
...
| Cross-compile, package, sign, and inspect the APK (entries including `assets/dashboard.html` and `assets/release_manifest.txt`, DEX classes, exports), lint's `NewApi` check that no library call exceeds the manifest's `minSdkVersion`, the built package's version read back from `aapt2 dump badging` against `APP_VERSION_CODE`/`APP_VERSION_NAME`, and the committed APK's entries (name, size, CRC-32) required to equal the fresh build's | `cargo xtask build-apk`, `cargo xtask verify-android-live` (`cargo xtask check-app-version`, a `gates` step, holds the bundled manifest and the APK's name to the same version) | Android SDK build-tools, platform `android.jar`, `cmdline-tools` (`lint`), NDK, JDK | yes (`android-apk` job installs the pinned platform, build-tools and NDK) |
...
| The app's real `ApiHttpServer` runs on the host JVM with fixture sources, a scripted `ScanControl` and the host `bleradar-jni` library: 27 real HTTP requests must be answered as documented (the dashboard bytes, the three JSON documents byte-identical to the browser fixtures, `404`/`405`/`400`, `no-store`, exact `Content-Length`; scan control — a pause reflected by every document, an accepted start, the four documented refusals as `409`, a throwing control answered `500` with the server still up, a 64 KiB request body consumed, a body declared beyond the 64 KiB cap refused with `413` at once), then headless Chromium must render the committed dashboard from that server with Start disabled and Stop offered; then the real `ReleaseManifestSource` must classify nine scripted fetches (a valid manifest, `404`, `503`, a `301` without `Location`, an oversized body, a rejected body, a stalled answer, a refused connection, a malformed URL) as documented, each dispositioned by the Rust core through the real `.so` | `cargo xtask verify-api-live` | a JDK (`javac`/`java`) and a Chromium/Chrome binary | yes (`gates` job, after `verify-jni-live`) |
| The app's unit tests (`android/app/src/test/java/com/hse/bleradar`: `BlipTest` — the RSSI window, the spread, the address-derived angle — and `ReleaseManifestTest` — the manifest contract through the real core; 64 `@Test` methods) executed on the host JVM with the host `bleradar-jni` library through xtask's generated JUnit-subset runner (`org.junit.Test`, the `Assert` overloads the tests call, a reflection runner): the native core must load (`ReleaseManifestTest`'s null-result half would pass vacuously without it), each listed class must run its pinned count with zero failures, and the directory may hold only the listed classes — until decision #98 every file there was dormant | `cargo xtask verify-android-unit` | a JDK (`javac`/`java`) | yes (`gates` job, after `verify-api-live`) |
| The committed APK on a real Android runtime: a throw-away AVD from the pinned API 34 `google_apis` x86_64 image (its ARM translation runs the arm64-only package) boots headless on KVM; the adapter is enabled and awaited, the APK installed with the runtime permissions granted, the activity launched, the loopback API forwarded; `GET /` must be the committed page byte for byte, the documents must carry their keys with `native_available` true, a start must answer `200` with `RadarScanService` promoted and its notification "BLE Radar is scanning", a stop must keep the foreground with "BLE Radar is idle", a virtual advertiser — a second controller connected to netsimd's HCI socket, advertising `bleradar-beacon` from `C0:DE:BE:AC:0D:01` — must be listed by `/api/devices` within 30 s with an RSSI and a finite distance (the Rust-computed row) and be pruned within 75 s of its removal, `kill -9` must be followed by a restarted service with the scan resumed, the first launch's update check must have reached its decision with its service finished (no record, no notification), `am force-stop` must end the API, a relaunch + scan + `pm revoke BLUETOOTH_SCAN` must leave no scan and no foreground service (the report names the branch the platform took), and logcat must carry no crash of the package and no leaked `ServiceConnection`; then the app must upgrade itself through its own pathway against a stand-in `github.com` the guest is made to trust (a `keytool` anchor in a shadowed Conscrypt store, a generated JDK `HttpsServer`, an xtask CONNECT proxy as the global proxy; the packages from `cargo xtask build-update-proof`): the production URL fetched `HTTP 200 -> using the remote manifest`, decision 1, the download, the core's verification and the installer hand-off within 120 s, the installer's `Update` tapped, the successor's versionCode installed within 90 s and its API up with the native core, the stand-in's log showing the redirect, the manifest and the artifact in order; the AVD is deleted on every exit | `cargo xtask verify-android-emulator` (`cargo xtask android-sdk-install --emulator` installs the SDK packages; `cargo xtask build-update-proof` writes the upgrade packages) | KVM (`/dev/kvm`), the SDK's `emulator` (which ships `netsimd`, the virtual radios), `platform-tools` and the pinned system image, a JDK for `avdmanager` and `keytool`, the SDK/NDK for the proof packages | yes (`android-emulator` job; 2 min 33 s on the first green run — decision #93; the revocation and update-check steps added in #94, proof step 1 min 54 s; the virtual advertiser in #95, proof step 2 min 12 s, job 4 min 00 s; the remote-manifest fetch in #96, proof step 1 min 58 s, job 3 min 32 s; the upgrade phase in #99 — handed to the installer 5.8 s after the launch, versionCode 2 installed 2.5 s after the tap; run 172 green: job 3 min 31 s, the upgrade phase 21 s) |
| BLE scan of real advertisers on a physical radio (the virtual advertiser proves the scan → Rust row path; a radio's RSSI physics is not simulated) and differential comparison against the oracle on a physical device (the update download/install path is exercised against a stand-in release host on the emulator, decision #99; the real `github.com` and a device's installer are not) | — | an ARM64 Android device with a radio, and the original signing key for update identity | no (MIG-003, narrowed by the emulator proofs; EXT-006) |
...
manifest/artifact surface — decision #87 — and 32 ↔ 32 after the
remote-manifest disposition — decision #96). Since decision #86 the
...
committed APK is the #98 build (SHA-256 `cc182b57…cbba`, 406,094 bytes;
`classes.dex` 75,956 bytes with `ReleaseManifest` logging through
`java.util.logging`; the native library byte-identical to the #96 build;
every entry stored at the ZIP epoch, so a rebuild on one signing key is
byte-identical, and `verify-android-live` requires the committed entries to
reproduce from the sources — decision #97, whose gate refused the #97
package on the first run after that Java change: `classes.dex: size 75864 →
75956, crc32 67ee51d9 → db6dd772`). The SDK set that build uses
...
and `dexdump -d` on both files is the way to tell the two apart. Since
decision #97 the same-host case is exact: `build-apk` stores every entry at
the ZIP epoch (the two staged files had carried the build's mtime, the only
difference between rebuilds), two builds on one signing key are
byte-identical, and `verify-android-live` requires the fresh build's entries
(name, size, CRC-32; `META-INF/` aside) to equal the committed APK's on every
pull request.
