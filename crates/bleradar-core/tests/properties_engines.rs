...
//! added after the original domain-core falsification sweep in
//! `properties.rs` (decision 21): metamorphic software advancement, adaptive
//! OSINT search, temporal infrastructure correlation, website lineage, and
//! calibrated evidence fusion. Same deterministic, dependency-free PRNG
//! methodology — true invariants over large randomized input sweeps, not
//! fixed examples — extended to the exact-rational priority formulas and
//! bounded calibration averages that were previously covered only by
//! hand-picked example tests.
...
/// Deterministic xorshift64 generator (same construction as `properties.rs`;
/// duplicated rather than shared across independent integration-test crates).
struct Rng(u64);
...
fn prop_temporal_interval_overlap_gap_and_contiguity_are_consistent() {
    ...
    for _ in 0..100_000 {
        ...
        let max_gap = rng.range_u64(0, 1_000_000);
        assert_eq!(
            left.is_contiguous_with(right, max_gap),
            gap_lr <= max_gap,
...
/// Nine calibrated dimensions used identically by [`EvidenceQuality`],
/// [`CorrelationFactors`] and [`WebsiteFactors`]; every value is drawn from
/// the full `u8` range (including above the 0..=100 calibration scale) so the
/// clamping each constructor performs is exercised, not just in-range input.
...
fn bump_one(dims: [u8; 9], index: usize, delta: u8) -> [u8; 9] {
    let mut bumped = dims;
...
fn prop_evidence_quality_calibrated_weight_is_bounded_and_monotonic() {
    ...
    for _ in 0..50_000 {
        let dims = random_dimensions(&mut rng);
        ...
        let higher = bump_one(dims, index, rng.range_u8(0, 50));
