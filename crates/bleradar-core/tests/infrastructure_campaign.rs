//! Random-operation campaign over
//! `TemporalMetamorphicInfrastructureCorrelationEngine`
//! (`docs/AUTONOMOUS_DECISIONS.md` #64): random `observe` / bulk-observe /
//! `correlate` / `correlate_all` / `ranked_correlations` sequences under random
//! limits and continuity windows, with colliding observation ids, conflicting
//! sources (same id, different metadata), a canonical observation pre-seeded
//! to conflict with an id the generators reuse, empty identifiers, unknown and
//! identical nodes, and bursts of up to 40 observations from distinct sources
//! so one correlation can carry 1,600 independent supports. Oracles after
//! every operation: no panic; observation and correlation counts never exceed
//! the limits; a refused operation leaves the observation set, the correlation
//! set, and the evidence store's length unchanged (`correlate_all` is
//! all-or-nothing); an accepted observation is retrievable, persisted, and
//! indexed under its node; an accepted correlation is persisted under the
...
//! nodes; after `correlate_all` every comparable pair is correlated in one
...
//! `BLERADAR_INFRASTRUCTURE_CAMPAIGN_SEQUENCES`, reseed with
//! `BLERADAR_INFRASTRUCTURE_CAMPAIGN_SEED`; a failure prints the seed.

use std::collections::BTreeSet;
use std::panic::{AssertUnwindSafe, catch_unwind};

use bleradar_core::{
    EvidenceStore, EvidenceValue, InfrastructureError, InfrastructureFactors, InfrastructureKind,
    InfrastructureLimits, InfrastructureObservation, Observation, RetrievalMethod, Source,
    SourceType, TemporalInterval, TemporalMetamorphicInfrastructureCorrelationEngine,
};

const DEFAULT_SEQUENCES: u64 = 100;
const OPS_PER_SEQUENCE: usize = 60;
const DEFAULT_SEED: u64 = 0x0517_2026_0910_1FFA;
...
fn observation(rng: &mut Rng, ts: u64) -> Option<InfrastructureObservation> {
    ...
    let kind = InfrastructureKind::ALL[rng.below(InfrastructureKind::ALL.len())];
    let value = VALUES[rng.below(VALUES.len())];
    let source = source(rng);
    let mut observation =
        InfrastructureObservation::with_timeline(id, node, kind, value, source, interval(rng, ts))
            .ok()?;
    if rng.chance(0.5) {
    ...
    if rng.chance(0.3) {
        ...
        observation =
            observation.with_factors(InfrastructureFactors::new(s, s, s, s, s, s, s, s, s));
    }
    if rng.chance(0.2) {
...
fn state(engine: &TemporalMetamorphicInfrastructureCorrelationEngine) -> State {
    State {
...
fn consistency(
    engine: &TemporalMetamorphicInfrastructureCorrelationEngine,
    ...
    for report in engine.correlations() {
        ...
        for id in edge.observation_ids() {
            ...
            if observation.node_id() != edge.subject() && observation.node_id() != edge.object() {
                return Err(format!("edge cites {id} from a third node"));
            }
        }
...
fn random_operation_campaign_keeps_the_engine_consistent() {
    let sequences = env_u64(
        "BLERADAR_INFRASTRUCTURE_CAMPAIGN_SEQUENCES",
        DEFAULT_SEQUENCES,
    );
    let seed = env_u64("BLERADAR_INFRASTRUCTURE_CAMPAIGN_SEED", DEFAULT_SEED);
    let mut rng = Rng(seed | 1);
    let mut successes = 0u64;
    let mut refusals = 0u64;
    let mut largest_support = 0usize;
    for seq in 0..sequences {
        let limits = InfrastructureLimits::new(1 + rng.below(80), 1 + rng.below(12))
            .unwrap()
            .with_maximum_temporal_gap([0u64, 5, 86_400_000][rng.below(3)]);
        let store = pre_seeded_store(&mut rng);
        let mut engine =
            TemporalMetamorphicInfrastructureCorrelationEngine::with_limits(store, limits);
        let mut ts = 100u64;
        for k in 0..OPS_PER_SEQUENCE {
            ...
            let result = catch_unwind(AssertUnwindSafe(|| -> Result<(), String> {
                if op < 5 {
                    ...
                    let node = observation.node_id().to_owned();
                    match engine.observe(observation) {
                        Ok(()) => {
                            ...
                            if engine.evidence().entity(&node).is_none() {
                                return Err(format!("node entity {node} not persisted"));
                            }
                            if !engine.observations_for_node(&node).any(|o| o.id() == id) {
                                return Err(format!("observation {id} not indexed under {node}"));
                            }
                        }
                        Err(error) => {
                            refusals += 1;
                    ...
                    let left = NODES[rng.below(NODES.len())];
                    ...
                    match engine.correlate(left, right) {
                        Ok(report) => {
                            ...
                            if id != format!("infrastructure-correlation:{left}:{right}") {
                            ...
                            if report.phases().len() != 8 {
                                return Err("report does not record eight phases".into());
                            }
                        }
                        Err(error) => {
                            refusals += 1;
                            ...
                            if matches!(error, InfrastructureError::SameNode { .. })
                                && left != right
                            {
                                return Err("SameNode for different nodes".into());
                            }
                        }
                    }
                } else if op < 11 {
                    match engine.correlate_all() {
                        Ok(reports) => {
                            ...
                            let nodes: BTreeSet<String> = engine
                                .observations()
                                .map(|o| o.node_id().to_owned())
                                .collect();
                            let nodes: Vec<String> = nodes.into_iter().collect();
                            for (i, left) in nodes.iter().enumerate() {
                                for right in nodes.iter().skip(i + 1) {
                                    let forward =
                                        format!("infrastructure-correlation:{left}:{right}");
                                    let reverse =
                                        format!("infrastructure-correlation:{right}:{left}");
