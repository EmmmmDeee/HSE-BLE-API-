...
//!
//! Ported from the Huntsman Search Engine (HSE) `hse-core::coords` module
//! (see `docs/HSE_IMPORT.md`). A radar ingests coordinates from wildly
//! inconsistent sources — a CSV column of signed decimals, a photo caption in
//! degrees-minutes-seconds, a ham-radio call log's Maidenhead locator, a
//! Google-Maps "Plus Code", a `geo:` URI from a vCard — so this module is the
//! single front door that accepts every notation and normalises it to the
//! crate's one coordinate type, [`crate::geo::LatLon`].
//!
...
//!
//! Every path funnels through [`crate::geo::LatLon::new`], so a returned
//! `ParsedCoordinate` is always finite and in range (`lat ∈ [-90, 90]`,
//! `lon ∈ [-180, 180]`). The entry point `parse` is **total**: it never
//! panics on any input and returns `None` when nothing parses.
//!
...
impl CoordFormat {
    ...
    /// `+` — that makes it safe to classify as a coordinate without
    /// false-positiving against ordinary handles or identifiers.
    ///
    ...
    /// self-evident: they are accepted only when the caller has already
    /// declared the value a coordinate.
    #[must_use]
...
/// A parsed WGS84 coordinate: a validated [`LatLon`] tagged with the
/// [`CoordFormat`] it was recognised from.
...
impl ParsedCoordinate {
    ...
    /// filtering is an output-policy concern for downstream consumers, not an
    /// input-parsing one.)
    fn checked(lat: f64, lon: f64, format: CoordFormat) -> Option<Self> {
        LatLon::new(lat, lon)
            .ok()
            .map(|point| Self { point, format })
    }
}

/// Parse any supported coordinate notation into a validated
/// [`ParsedCoordinate`].
///
...
///
/// ```
/// use bleradar_core::coords::{CoordFormat, parse};
///
/// let p = parse("27°28'35.8\"S 153°00'59.8\"E").expect("should succeed");
/// assert!((p.point.lat() - -27.476611).abs() < 1e-5);
/// assert!((p.point.lon() - 153.016611).abs() < 1e-5);
/// assert_eq!(p.format, CoordFormat::Dms);
///
/// assert!(parse("not a coordinate").is_none());
/// ```
#[must_use]
pub fn parse(input: &str) -> Option<ParsedCoordinate> {
    let s = input.trim();
...
fn parse_geo_uri(s: &str) -> Option<ParsedCoordinate> {
    ...
    ParsedCoordinate::checked(lat, lon, CoordFormat::GeoUri)
}
...
fn parse_decimal_pair(s: &str) -> Option<ParsedCoordinate> {
    let (a, b) = split_pair(s)?;
    let lat: f64 = a.trim().parse().ok()?;
    let lon: f64 = b.trim().parse().ok()?;
    ParsedCoordinate::checked(lat, lon, CoordFormat::Decimal)
}
...
fn parse_dms(s: &str) -> Option<ParsedCoordinate> {
...
fn parse_dms_single_prefix_hemisphere(s: &str) -> Option<ParsedCoordinate> {
...
/// Combine two parsed sides into a validated [`ParsedCoordinate`], honouring
/// N/S/E/W axis assignment and rejecting contradictions (two latitudes, etc.).
fn combine(lhs: Side, rhs: Side) -> Option<ParsedCoordinate> {
    ...
    ParsedCoordinate::checked(lat_side.value, lon_side.value, format)
}
...
fn parse_dms_halved(s: &str) -> Option<ParsedCoordinate> {
    let tokens = scan_numbers(s);
    let n = tokens.len();
    if n < 2 || !n.is_multiple_of(2) {
        return None;
...
fn parse_maidenhead(s: &str) -> Option<ParsedCoordinate> {
    let t = s.trim();
...
fn parse_plus_code(s: &str) -> Option<ParsedCoordinate> {
    let t = s.trim();
